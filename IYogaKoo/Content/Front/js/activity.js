$(function () {
    ...
    $(document).on('click', '.addInterest', function () {
        ...
        if (hidUid != "" && hidUid != "0") {
            ...
            if (confirm("登录后才可以添加感兴趣的活动!")) {
                window.location = "/Login/Login?ReturnUrl=" + '/Class/Apply?id=' + $('#hidid').val();// 新窗口
            }
            // ShowError('没有活动');
        }
    });
    ...
    $('body').on("submit", "form#add-class-teacher", function () {
        ...
        if ($("#avatar-img").attr("src") == undefined) {
            ShowError("请选择图片");
            return false;
        }
        if ($('#add-class-teacher').valid()) {
            var options = {
    ...
    $('#add-activity').submit(function () {
        ...
        if ($('#add-activity').valid()) {
            var options = {
                success: function (json) {
                    layer.close(addSapceLayIndex);
                    if (json.Id > 0) {
                        window.location.href = "/class/AddActivityBanner?id=" + json.Id;
                    }
                    else {
                        $('.error').html("添加失败，请重试");
                    }
                },
                url: '/class/AddActivity',
                dataType: 'json'
            };
    ...
    Apply: function () {
        ...
        PostData('/class/apply', apply, function (json) {
            ...
            if (json.Code == 0) {
                ...
                $.post('/class/Verify?id=' + json.Obj.Id, "", function (data) {

                    if (data.Code == 0 && data.Obj == 0) {
                        layer.confirm('报名成功', {
                            btn: ['确定'] //按钮
                        }, function () {
                            time: 0;
                            window.location.href = "/class/viewactivity?id=" + json.Obj.ClassId;
                        });
                    }
                    else if (data.Code == 0 && data.Obj == 1) {
                        layer.confirm('报名成功', {
                            btn: ['支付'] //按钮
                        }, function () {
                            time: 0;
                            window.location.href = "/class/WXNotify?id=" + json.Obj.Id;
                        });
                    }
                    else {
...
function ShowBox(url, title, param, callback) {
    ...
    $.get(url, param, function (html) {
        Loaded();
        layer.open({ type: 1, area: ['auto', '475px'], title: title, shadeClose: false, content: html });
...
function LoadClassTeacher(page) {
    SearchList({
        url: '/class/getyogis', data: { text: $('.search-teacher input:text').val(), page: page, size: 5 }, row: function (dataItem) {
...
function LoadClass(page, code, args) {
    SearchList({
        url: '/class/index', data: { page: page, size: 12, code: code, args: args }, row: function (dataItem) {
            ...
            if (args.split(',')[2] == 3)
                html += '<a href="/class/ActivityReportDetailsPage?id=' + dataItem.Id + '&&classReportId=-99" class="banner"><img src="' + dataItem.Banner + '"></a>';
            else
            ...
            if (args.split(',')[2] == 3)
                html += '<a href="/class/ActivityReportDetailsPage?id=' + dataItem.Id + '&&classReportId=-99" class=""><h3>' + dataItem.Name + '</h3></a>';
            else
                html += '<a href="/class/viewactivity?id=' + dataItem.Id + '" class=""><h3>' + dataItem.Name + '</h3></a>';
            var areas = dataItem.NoPassMsg.split('-');
            html += '<p><b style="width:60px;">活动时间： </b><span style="width:80px;">' + dataItem.Start + '</span><b style="width:60px;">活动地点：</b> <span style="Minwidth:60px;">' + areas[3] + areas[4] + '</span><b style="width:60px;">活动费用：</b> <span>' + dataItem.Price + '</span></p>';
            ...
            //html += '<p><p class="p1">' + dataItem.User.NickName + '   </p></p>';
            html += '<p><p class="p2"><a href="">' + dataItem.InterestCount + '</a>人感兴趣 ' + dataItem.Max + '人参加</p></p>';
            html += '</div>';
            html += '</div>';
            html += '</div>';
            return html;
...
function LoadReport(page, code, args) {
    SearchList({
        url: '/class/Reports', data: { page: page, size: 12, code: code, args: args }, row: function (dataItem) {
            ...
            html += '<a href="/class/viewreport?classid=' + dataItem.Id + '"><img src="' + dataItem.Banner + '"></a>';
            html += '</div>';
            html += '<div class="lb_lb_hd_index_show_img2_bott">';
            html += '<p><a href="/class/viewreport?classid=' + dataItem.Id + '">' + dataItem.Name + '</a></p>';
            html += '<p><a href="/class/viewreport?classid=' + dataItem.Id + '">' + dataItem.Content + '&nbsp;' + dataItem.Start + '</a><span><a>42</a>人参加</span></p>';
            html += '</div>';
            html += '</div>';
            return html;
